...
Do many times:
    ...
    Select a duration between (min,max) sec at random.
    Add this info to playlist file.
Execute VLC once and pass it the xml as a playlist.
...
for q in range(numclips):
    ...
    subpath = os.path.join(THIS_FOLDER, subfolder) 
    videofile = os.path.join(subpath, Videos[selected]).replace('\\', '/') 
    ...
    # Select a starting point from begining of video to end of video - max:
    ...
    ET.SubElement(track, "location").text = "file:///" + videofile
...
executable = os.path.join(THIS_FOLDER, 'exevlc.bat') 
p = subprocess.Popen([executable, THIS_FOLDER + '/' + xmlplaylistfile])
